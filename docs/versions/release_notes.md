...
Note that this is the last full version released before the full release of MPF 0.80.0.
This means that it is the best version to upgrade to as a last step _before_ upgrading to 0.80.0.
 ...
 * random_event_players using conditional events that all drop out, and a fallback event, will fall back correctly
 * random_event_players using conditional events that all drop out, and no fallback event, will fall back to the last event in the conditional list (even though it is false)
 * fix kickback devices crashing in virtual platform when using coil_pulse_delay.
