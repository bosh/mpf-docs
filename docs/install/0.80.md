...
The newest release of MPF is 0.80, which includes support for the Godot Media Controller (GMC) and drops support for the 0.57 Kivy-based Media Controller (MC). If you previously have installed a dev version of 0.80, you can search this guide for changes listed as being from higher dev versions than what you have installed, as you will need to follow those steps as well.

While there are not many functional changes in the core pinball functionality of MPF between 0.57.5 and 0.80.0,
there are some config file migrations necessary and this document will help you through that.
...
* Version Compatibility
    * [Python Version](#python-version-compatibility) - minimum now Python 3.10
    * [MPF-GMC Version 0.1.6+](#mpf-and-gmc-version-compatibility) - MPF `0.80.0.dev9` will automatically enforce version compatibility with MPF-GMC.
* Media Controller components
    * [Slide and Widgets](#slides-and-widgets) - config sections deprecated
    * [Sound Player](#sound-player) - see [GMC: Sound](../gmc/sound.md)
    * [Sound Pools](#sound-pools) - replaced by Godot `AudioStreamRandomizer` nodes
    * [Deprecated Config Sections](#deprecated-config-sections) - `0.80.0.dev12` enforces that unsupported config sections are removed
* Changes to Built-In/Default Modes
    * [Bonus Mode](#bonus-mode) - Changed in `0.80.0.dev1`
    * [Carousel Modes](#carousel-modes) - `0.80.0.dev10` changes event naming conventions
    * [High Score Mode](#high-score-mode) - `0.80.0.dev10` changes event naming conventions
    * [Tilt Mode](#tilt-mode) - Default slide added in GMC `0.1.4`, behaviors changed in MPF `0.80.0.dev8` 

## Software version compatibility

### Python version compatibility

MPF 0.80 moves the compatible Python range forward to match the official supported Python versions per [python.org](https://devguide.python.org/versions/). Versions 3.8 and 3.9 are no longer supported, but Python versions 3.13 and 3.14 are now officially supported by MPF. If you need to install a newer version of Python, you will also need to reinstall MPF via the matching version of pip.
...
The config sections used by MPF-MC are deprecated with the upgrade to `0.80.0.dev12` and must be removed.
These include the following configs:
...
**updated in `0.80.0.dev10`**

Carousel events have changed to use a consolidated *carousel_item_highlighted* and *carousel_item_selected* event instead of unique events for each item. You must use conditional event logic to select the correct events for your config_players.

For example, given a carousel mode "missionselect" and an item "garrus":
...
If you wish to customize your tilt slides, see the `slide_player:` entry in the MPF codebase file [mpf/modes/tilt/config/tilt.yaml](https://github.com/missionpinball/mpf/blob/0.80.x/mpf/modes/tilt/config/tilt.yaml) for examples.
