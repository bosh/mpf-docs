...
code, so in this step we're going to focus on getting your flippers working.
...
[combo switch](../../game_logic/combo_switches.md) feature which posts
events when player switches are held in
combination. If you add these tags to your flipper switches, an event
called [*flipper_cancel*](../../events/flipper_cancel.md) will be
posted when the player hits both flipper
...
know about YAML files (which is the format of the file we're creating here):

* You cannot use tabs to indent in YAML. (It is [literally not allowed](http://www.yaml.org/faq.html).)
    Most text editors can be configured to automatically insert spaces when you push the tab key,
...
switches:
  s_left_flipper:
    number: 0  # this can be blank if you don't have physical hardware yet
  s_right_flipper:
    number: 1  # if you do have physical hardware, most likely your number will be different
```
...
Remember earlier in this step, we provided links to the documentation for each platform. Here is
...
for your own use, rather, follow the instructions from the link to the list above.
...
In case you are using [the Virtual Pinball (VPX) Platform](../../hardware/virtual/virtual_pinball_vpx.md)
...
* If you're using a P-ROC and you get a bunch of really fast messages
    about `Error opening P-ROC device` and `Failed, trying again...`,
    this is because (1) your pinball machine is not turned on,
    (2) your P-ROC is not connected to your computer (via USB),
    or (3) you have a problem with the P-ROC drivers. If you're
